...
pub fn run() {
    ...
    println!("{}", usize::BITS);
    println!("Memory addres of const is: {:p}", &MAX_POIONTS);
    ...
    *y1_ptr = -5;
    ...
    let s1 = "helleこんにちは挨拶"; // 26bytes
    let s2 = "helle"; // 5bytes
    println!("Stack address of s1 is: {:p}", &s1);
    println!("Stack address of s2 is: {:p}", &s2);
    println!("Static memory addres of s1 is: {:?}", s1.as_ptr());
    println!("Static memory addres of s2 is: {:?}", s2.as_ptr());
    ...
    println!("Stack address of s1 is: {:p}", &s1);
    println!("Stack address of s2 is: {:p}", &s2);
    println!("Heap memory addres of s1 is: {:?}", s1.as_ptr());
    println!("Heap memory addres of s2 is: {:?}", s2.as_ptr());
    println!("Len of s1 is: {:?}", &s1.len());
    println!("Len of s2 is: {:?}", &s2.len());
